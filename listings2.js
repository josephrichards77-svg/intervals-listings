...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function formatFullDate(date) {
        ...
        return `${date.toLocaleDateString("en-GB", { weekday: "long" })}, ${date.toLocaleDateString("en-GB", { month: "long" })} ${day}${getOrdinal(day)}, ${date.getFullYear()}`;
    }
        ...
        fetch(SHEET_URL)
            ...
            .then(sheet => {
                ...
                sheet.values.slice(1).forEach(row => {
                    ...
                    // ✅ FIXED FILTER (substring match)
                    if (
                        venue &&
                        !cinema.toLowerCase().includes(venue.toLowerCase())
                    ) return;
                ...
                Object.keys(data)
                    .sort((a, b) =>
                        normaliseCinemaName(a).localeCompare(normaliseCinemaName(b))
                    )
                    .forEach(cinema => {
